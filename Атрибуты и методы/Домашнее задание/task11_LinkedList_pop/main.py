...
class LinkedList:
    ...
    def index(self, value: Any, start: Optional[int] = None, stop: Optional[int] = None) -> int:
        ...
        if start is not None and not 0 <= start < (self.len - 1):
            raise IndexError("значение индекса выходит за диапазон списка")

        if stop is not None and not 0 <= stop < (self.len - 1):
            raise IndexError("значение индекса выходит за диапазон списка")

        if stop is not None and start is not None and stop <= start:
            raise IndexError("начало диапазона поиска не должно ровнятся или превышать его конец")
    ...
    def extend(self, data: Optional[Iterable] = None) -> None:
    ...
    def pop(self, index: Optional[int] = None) -> None:
        ...
        if index is None:
            print(self.step_by_step_on_nodes(self.len - 1))
